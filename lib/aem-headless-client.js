...
class AemHeadlessClient {
   ...
   */
   async getChallengesStepContent(challengeName, step, challengesList) {
    console.log(challengeName, step, challengesList, 'testing')
    let challengesPaths = [];
    try {
    ...
    const challengeStepPath = challengesPaths.stepsPaths.filter((val) => {
      return val.title == step[0].toUpperCase().replace('-', ' ');
    });
    const queryChallengeByName = challengeStepPath.length ? `StayingSharpContentFragments/find-step-by-path${(encodeURIComponent(`;fragmentPath=${challengeStepPath[0]._path}`))}` :
     '';
