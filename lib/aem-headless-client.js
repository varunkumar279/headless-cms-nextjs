...
class AemHeadlessClient {
  ...
  async getChallengesStepsByName(challengeName, challengesList) {
    ...
    const queryChallengeByName = `StayingSharpContentFragments/find-challenge-by-path${(encodeURIComponent(`;fragmentPath=${challengePath[0]._path}`))}`;
    console.log(queryChallengeByName, 'query');
    const queryVariables = {'challengeLanding': challengeName};
    // const queryChallengeByName = process.env.NEXT_PUBLIC_AEM_GRAPHQL_ENDPOINT + '/adventure-by-slug';

    try {
      return await this.aemHeadlessClient.runPersistedQuery(queryChallengeByName, queryVariables);
    } catch(e) {
      console.error(e);
    }
  }
